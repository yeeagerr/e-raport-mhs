<form class="relative w-96 h-[500px]" action="{{route('update_siswa_post', $user->id)}}" method="POST">
    @csrf
    @method("PUT")
    <div
    ...
    <div
        ...
        <input
            ...
            type="nama" name="nama" value="{{$id->nama}}" />
        <div
        ...
        <input
            class="bg-white-200 text-gray-700 focus:outline-none focus:shadow-outline border border-gray-300 rounded-3xl py-2 px-4 block w-[60%] h-[7%] appearance-none relative z-10"
            type="username" name="username" value="{{$id->username}}" />
        <div
        ...
        <div
            class="absolute top-[165px] left-15 -ml-4 w-[70%] h-[9%] bg-red-800 rounded-3xl shadow z-0   max-sm:-ml-1  md:-ml-1">
        </div>
        ...
        <input
            class="bg-white-200 text-gray-700 focus:outline-none focus:shadow-outline border border-gray-300 rounded-3xl py-2 px-4 block w-[60%] h-[7%] appearance-none relative z-10"
            type="text" value="{{$id->nuptk}}" name="nuptk" />
        <div
            class="absolute top-[230px] left-15 -ml-4 w-[70%] h-[9%] bg-red-800 rounded-3xl shadow z-0   max-sm:-ml-1  md:-ml-1">
        </div>
        @else
        <label class="mt-2 block text-gray-700 text-sm font-bold mb-2">NUPTK</label>
        <input
        ...
        <select
            ...
            @foreach ($kelas as $i)
            <option value="{{$i->id}}" {{$id->kela_id == $i->id ? "selected" :
                ""}} > {{$i->nama_kelas}}</option>
            @endforeach
        </select>
        <div
            class="absolute top-[297px] left-15 -ml-4 w-[70%] h-[9%] bg-red-800 rounded-3xl shadow z-0   max-sm:-ml-1  md:-ml-1">
        </div>
        @endif
